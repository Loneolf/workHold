...
// GBK Byter转字符串
function gbkbin2String(array) {
    return gbk.default.decode(array)
}
/*
  把byter转成数字
*/
...
// 浮点数计算方式
var getFloat = function getFloat(arr) {
    var a = arr[0];
    var b = arr[1];
    var c = arr[2];
    var d = arr[3];
    ...
    c = c | 128;
    var cl = (c << (size - 7)) < 0 ? ((c << (size - 7)) >>> 0) : (c << (size - 7));
    var bl = (b << (size - 15)) < 0 ? ((b << (size - 15)) >>> 0) : (b << (size - 15));
    var al = (a << (size - 23)) < 0 ? ((a << (size - 23)) >>> 0) : (a << (size - 23))
    ...
    if (size <= 7) {
        ...
        temp = (a >> (23 - size)) + bl + cl;
    } else if (size > 23) {
        // temp = (a << (size - 23)) + (b << (size - 15)) + (c << (size - 7));
        temp = al + bl + cl;
    }
