...
function drawGdbd(options){
    this.option = {
        ...
        yInfo:{
            ...
            rightFontColor:$("body").attr('class').indexOf('theme-dark') != -1?'#787878':'#777',
            fontSize:'26',
            kdTextNum:'3', //4个刻度文字
            unit:''
        },
 ...
 */
drawGdbd.prototype.init = function(){
    var _this = this;
 ...
 * @return {[type]} [description]
 */
drawGdbd.prototype.extendCanvas = function(){
    //绘制虚线
...
*@param cnst canvas对象
*/
drawGdbd.prototype.drawZBX = function(cnstContext,cnst){
    var _this = this,
    ...
    yKdMaxC = TOOLS.format(yKdMaxC/10000,2)+_this.option.yInfo.unit;
    ...
    //判断最大值和最小值
    if(Number(_this.option.data.rectangle.minValue)===0 && Number(_this.option.data.rectangle.maxValue)===0){
        _this.option.data.rectangle.minValue = 0;
        _this.option.data.rectangle.maxValue = (_this.option.yInfo.kdTextNum-1)*0.1;
    }
    ...
    for(var i=0;i<_this.option.yInfo.kdTextNum;i++){
        var nY = basicY - i*yStep,
            sText = TOOLS.format((Number(yMinValue)+Number(i*yStValue))/10000,2),
            oFont = {
    ...
    //判断最大值和最小值
    if(Number(_this.option.data.brokenLine.minValue)===0 && Number(_this.option.data.brokenLine.maxValue)===0){
        _this.option.data.brokenLine.minValue = 0;
        _this.option.data.brokenLine.maxValue = (_this.option.yInfo.kdTextNum-1)*1;
    }
    var yRightTextWidth = _this.getFontWidth(cnstContext,TOOLS.format(_this.option.data.brokenLine.maxValue,0),_this.option.yInfo.fontSize);
    ...
    for(var i=0;i<_this.option.yInfo.kdTextNum;i++){
        var nY = basicY - i*yStep,
            sText = TOOLS.format((Number(yMinValue)+Number(i*yStValue)),0),
            oFont = {
                fontX:cnst.width - TOOLS.getARRType(_this.aPadding,3)+_this.option.yInfo.fontSize/2,
                fontY:nY,
                fontColor:_this.option.yInfo.rightFontColor,
                fontSize:_this.option.yInfo.fontSize,
                fontText:sText
            };
        _this.drawFillText(cnstContext,oFont,'end');
 ...
 * @return {[type]}     [description]
 */
drawGdbd.prototype.drawJx = function(cnstContext,cnst){
    var _this = this,
        nHeight = cnst.height-TOOLS.getARRType(_this.aPadding,4),
 ...
 * @param cnst canvas对象
 */
drawGdbd.prototype.drawZLine = function(cnstContext,cnst){
    var _this = this,
...
*/
drawGdbd.prototype.drawFillText = function(ctx,oFont,sTextAlign){
...
*@param fontSize 文字大小
*/
drawGdbd.prototype.getFontWidth = function(ctx,strText,fontSize){
...
/*@action 监听触摸动作
*/
drawGdbd.prototype.eventTouch = function(){
    var _this = this,
        nX = 0,
        qDianX = 0,
        oTouchPoint = {};
...
*@param fn 回调函数
*/
drawGdbd.prototype.mouseMoveEvent=function(x,fn,flag) {
    var _this = this;
    ...
    _this.crossContext.beginPath();
    _this.crossContext.moveTo(nX-80,nY+40);
    _this.crossContext.lineTo(nX+80,nY+40);
    _this.crossContext.lineTo(nX+80,nY-40);
    _this.crossContext.lineTo(nX-80,nY-40);
    _this.crossContext.lineTo(nX-80,nY+40);
    ...
    var oDian = _this.option.data.rectangle.yData[xNum],
        oFont = {
            fontSize:'24',
            fontColor:'#ffffff',
            fontText:TOOLS.unitHuFun(oDian['value'],2)+'户',
            fontX:nX,
            fontY:nY-12
        }
    _this.drawFillText(_this.crossContext,oFont,'center');
    if(!!oDian['price']){
        oFont.fontText = TOOLS.format(oDian['price'],2)+'元';
...
var TOOLS = {
    ...
    */
    ...
    unitHuFun:function(num,dig){
        if(isNaN(num)) return '--';
        if(!isNaN(num) && Number(num)<10000){
            return Number(num);
        }
        return this.fmtunit(num,dig);
    },
    /**
