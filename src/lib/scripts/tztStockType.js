...
const MakeKCBMarket = function(stockProp)
{
  ...
  if (stockProp.length < 32)
    ...
    if (nsStock === '0' && nsKCB == '7' || nsStock === '0' && nsKCB == '3' || nsStock === '6' && nsKCB == '2') {//07科创板、03创业板、62科创板cdr或创业板cdr
      return true;
    }
    return false;
...
//新三板可转债
function tztZZMakeThreeBordConvertibleBondMarket(x) {
  return (MakeMarket(x) ==HQ_STOCK_MARKET&& MakeSubMarket(x) == HQ_KIND_KCB && MakeMidMarket(x) == HQ_SZ_BOURSE);
}

function isMakeMarket(x) { return (MakeMarket(x) == HQ_STOCK_MARKET && !(MakeSubMarket(x) == HQ_KIND_INDEX))}

// 上海市场 - (不包含基金,债券)
function isSHMarketStock (x) { 
  return (isMakeMarket(x) && MakeMidMarket(x) == HQ_SH_BOURSE) && MakeSubMarket(x) !== HQ_KIND_BOND
    && MakeSubMarket(x) !== HQ_KIND_FUND
    && MakeSubMarket(x) !== HQ_KIND_LOF
    && MakeSubMarket(x) !== HQ_KIND_ETF 
    && !tztZZMakeThreeBordConvertibleBondMarket(x)
};
// 深证市场 - (不包含基金,债券)
function isSZMarketStock (x) { 
  return (isMakeMarket(x) && MakeMidMarket(x) == HQ_SZ_BOURSE) && MakeSubMarket(x) !== HQ_KIND_BOND 
    && MakeSubMarket(x) !== HQ_KIND_FUND
    && MakeSubMarket(x) !== HQ_KIND_LOF
    && MakeSubMarket(x) !== HQ_KIND_ETF 
    && !tztZZMakeThreeBordConvertibleBondMarket(x)
...
// 北交所债券现券
function MakeJsgz_hs(x){
  return MakeMarket(x) == '0X1000' && MakeMidMarket(x) == HQ_SH_BOURSE
}

function MakeJsgz_sz(x){
  return MakeMarket(x) == '0X1000' && MakeMidMarket(x) == HQ_SZ_BOURSE
}


// 沪深市场
function kuoWeiHu(x) {return (isMakeMarket(x) && MakeMidMarket(x) == HQ_SH_BOURSE)}
function kuoWeiShen(x) {return (isMakeMarket(x) && MakeMidMarket(x) == HQ_SZ_BOURSE)}
export default {
    kuoWeiShen:kuoWeiShen,
    kuoWeiHu:kuoWeiHu,
    isSZMarketStock:isSZMarketStock,
    isSHMarketStock:isSHMarketStock,
    ...
    MakeKCBMarket: MakeKCBMarket,
    MakeJsgz_hs:MakeJsgz_hs,
    MakeJsgz_sz:MakeJsgz_sz
}
