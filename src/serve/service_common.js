...
class tradeService {
  getData(obj,oSendData,oConfig){
    return new Promise((resolve, reject) => {
      ...
      if(/*browser.webview &&*/H5TZT){
          config.fnSuccess = function(oData){
            resolve && resolve(oData);
          }
          H5TZT.getData(config);
      }else{
          ...
          xhr({url:newUrl,body:newData}).then((oData)=>{
            ...
            //   let sAction = config.oSendData.action;
            //   if(!sAction){
            //     sAction = config.oSendData.Action;
            //     sAction = (!!sAction?sAction:config.oSendData.ACTION);
            //   }
            //   alert("请求超时["+sAction+"]");
            //   return;
            // }
