...
const numTosize2 = function (num,pos) {
  if(!num){
    return num;
  }
  pos = pos || 2;
  ...
  if(floatNum>100000 && floatNum<100000000){
    return toDecimal2(floatNum/10000*f,pos).replace(reg,'')+'万'
  }

  if(floatNum>100000000){
    return toDecimal2(floatNum/100000000*f,pos).replace(reg,'')+'亿'
  }
  return toDecimal2(floatNum*f,0);
};
...
const removeLocalStorage = function (array) {
  array.forEach((item,index)=>{
...
const padNumber = function(num, fill) {
  var len = ('' + num).length;
  return (Array(
