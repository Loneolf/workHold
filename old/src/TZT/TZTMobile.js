...
export default {
  ...
  /**
   * [action10408 统一登录退出]
   * @param  {string} url 定制的url
   * @param  {string} txt 退出提示文案
   */
  action10408(url,txt){
    var copyArr = deal_service_config.tztRemoveKey.slice();
    localService.removemapMesg(['tztrzrqloginflag','jyloginflag']);
    localService.removemapMesg(copyArr,{logintype:2});
    localService.removemapMesg(copyArr,{logintype:9});
    localService.removemapMesg(copyArr);
    var sUrl = '/deal/index',sTxt = '交易登录已经退出!';
    if(!!url) sUrl = url;
    if(!!txt) sTxt = txt;
    router.go(sUrl);
    $.alert(sTxt);
    ...
    localService.readMapMesg(['jyloginflag','tztrzrqloginflag','logintype='+config.logintype])
      .then((data)=>{
        ...
        if((config.logintype ==1 && jyloginflag >= 1) || (config.logintype ==2 && tztrzrqloginflag >= 1)){
          if(!!config['isCheckLoginRight'] && config['isCheckLoginRight'] == 'ggt'){
            localService.readMapMesg(['ISHKACCOUNT','ISHKSZACCOUNT','logintype=1']).then((data)=>{
              if(data.ISHKACCOUNT=='0' && data.ISHKSZACCOUNT=='0'){
                localService.removemapMesg(['footerStockcode','footerWtaccountType']);
                $.alert(deal_service_config.TIPINFO['ggt']);
                return;
              }else{
                continueGo();
              }
          ...
          var sPath = '/deal/login?logintype='+config.logintype;
          // if(!!config.fuc) sPath += '&fc='+config.fuc; //回调跳转有问题暂时屏蔽掉
          if(!!config['isCheckLoginRight']) sPath += '&loginRight='+config['isCheckLoginRight']; //回调跳转有问题暂时屏蔽掉
          if(config.goback){
            router.go({path:sPath,replace:true});
   ...
   * @param string paramFlag 传入父组件的参数是对象=true
   * */
  tradeaction(obj,_this,paramFlag){
    var config = {
      title:'',
      stockcode:'',
      url:'',
    ...
    if(typeof obj.isExport == core_strundefined || !obj.isExport){
      ...
      if(config.action in deal_service_config.tradeaction){
        ...
        if(_this && url.indexOf(_this.$route.matched[0].handler.fullPath)>-1&& config.stockcode != '' && _this.viewname && _this.viewname == oAction['name']){
          var obj = config.stockcode;
          var wtaccounttype = config.wtaccounttype||''; //容错处理
          if(typeof paramFlag != 'undefined' && paramFlag == true) obj = config;
          _this.$dispatch('updateStock', obj,wtaccounttype);
          return;
        }
      }
      ...
      let oObj = {url:url,logintype:logintype};
      if(!!config.isCheckLoginRight){
        oObj['isCheckLoginRight'] = config.isCheckLoginRight;
      }
      this.action10090(oObj);
    }else{
      return exportUrl;
    }
  },
  //下面这个几个方法用于自定义事件，触发，移除自定义事件
  /**
   * [addEvent 添加自定义事件]
  ...
  addEvent(type,fn,capture){
    ...
    if (window.addEventListener) {
        ...
        // 在元素上存储创建的事件，方便自定义触发
        if (!el["ev" + type]) el["ev" + type] = ev;
    } else if (window.attachEvent) {
        el.attachEvent("on" + type, fn);    
        if (isNaN(el["cu" + type])) { // 自定义属性，触发事件用
            el["cu" + type] = 0; 
        }
