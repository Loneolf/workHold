...
(function($) {
    ...
    $.touchEvents = {
        ...
        touchcancel: $.support.touch ? 'touchcancel' : 'mousecancel'
    };
...
+function ($) {
    ...
    $.openModal = function (modal, cb) {
        ...
        var nobackground = modal.hasClass('nobackground');
    ...
    var Picker = function (params) {
        ...
        p.initPickerCol = function (colElement, updateItems) {
            ...
            col.updateItems = function (activeIndex, translate, transition, valueCallbacks) {
                // console.log("activeIndex：" + activeIndex + "translate：" + translate + "transition：" + transition + "valueCallbacks:" + valueCallbacks);
            ...
            function handleTouchEnd (e) {
                ...
                if (touchEndTime - touchStartTime > 300) {
                    ...
                    touchCurrentY = parseInt(e.changedTouches[0].clientY);
                    var diff = touchCurrentY - touchStartY;
                    newTranslate = startTranslate + diff;

                    // console.log("velocityTranslate：" + velocityTranslate + "momentumRatio：" + p.params.momentumRatio);
                ...
                // console.log("currentTranslate1：" + newTranslate);
                // Update items
            ...
            col.initEvents = function (detach) {
                ...
                col.container[method]($.touchEvents.touchcancel, handleTouchEnd);
        ...
        function openOnInput(e) {
            ...
            console.log("p.opened：" + p.opened);
            if (p.opened||p.openedload) return;
            p.open();
            if (p.params.scrollToInput) {
                var pageContent = p.input.parents('.content');
                if (pageContent.length === 0) return;
        ...
        function onPickerClose() {
            ...
            p.params.callback && p.params.callback(p.value.join(''));
        }

        p.opened = false;
        p.openedload = false; //原有opened判断不准确，opened事件改变opened值，存在时差
        p.open = function () {
            if (!p.opened) {
                p.openedload = true;
                // Layout
...
+ function(window) {
    ...
    IScroll.prototype = {
        ...
        refresh: function() {
            ...
            //childrenDom特殊定义 修改时间：20170411 修改人：hhz
            this.wrapperWidth = (!!this.options.myWapperWidth?this.options.myWapperWidth:this.wrapper.clientWidth);
...
+ function($) {
    ...
    $.initPullToRefresh = function(pageContainer) {
        ...
        var isTouched, isMoved, touchesStart = {},
            ...
            useTranslate = false,
            startTranslate = 0,
            isUp = eventsTarget[0].getAttribute('data-up'),
            translate, scrollTop, wasScrolled, triggerDistance, dynamicTriggerDistance,eventsHeight,isType;
        isType = '';
...
console.log(isUp);
        ...
        function handleTouchMove(e) {
            ...
            if (touchesDiff > 0 && scrollTop <= 0 || scrollTop < 0) {
              ...
              isType = 'pull';
            }else if(touchesDiff < 0 && (container[0].scrollHeight-scrollTop-eventsHeight<=0) && isUp == 1){
              if ($.device.ios && parseInt($.device.osVersion.split('.')[0], 10) > 7 && scrollTop === 0 && !wasScrolled) useTranslate = true;
              var diff = Math.abs(touchesDiff);
              if (useTranslate) {
                e.preventDefault();
                translate = (Math.pow(diff, 0.85) + startTranslate);
                container.transform('translate3d(0,-' + translate + 'px,0)');
              } else {}
              if ((useTranslate && Math.pow(diff, 0.85) > triggerDistance) || (!useTranslate && diff >= triggerDistance * 2)) {
                refresh = true;
                container.addClass('pull-up').removeClass('pull-down');
              } else {
                refresh = false;
            ...
            //解决下拉滑动距离超过内容高度时，无法还原的问题
            var documentHeight=eventsTarget.height();
            if(pageY>=documentHeight){
                if (!isTouched || !isMoved) {
                    isTouched = false;
                    isMoved = false;
                    return;
        ...
        function handleTouchEnd() {
            ...
            if (refresh) {
                ...
                if(container.hasClass('refreshing')) return;
                container.addClass('refreshing');
                container.trigger('refresh',isType);
            } else {
                container.removeClass('pull-down');
            }
            isTouched = false;
...
//重写number的toFixed方法 各个浏览的四舍五入中，临界值5，有些浏览器直接舍去了，没有入。
+ function() {
    "use strict";
