        ...
        <div class="cont_left list-block ">
          ...
          <div class="list-abroad margin-bottom">
            ...
            <div class="select_drop" v-show="isStockList">
              <ul>
                <li v-for="items in stockList" @click.stop="stockListFunc($index)"><a href="javascript:void(0);"><strong style="font-size:0.75rem">{{items.STOCKCODEINDEX}}</strong>{{items.STOCKNAMEINDEX}}</a></li>
              </ul>
            </div>
            ...
            <div class="select_drop" v-show="isAppointList">
              <ul>
                <li v-for="items in appointData" @click="orderFunc($index)"><a href="javascript:void(0);" style="font-size: 0.4rem;">{{items.CONTACTINDEX}}-{{items.STOCKNAMEINDEX}}</a></li>
              </ul>
            </div>
          </div>
          <div class="list-abroad margin-bottom select_box2">
          ...
          <div class="btn-wrapp">
            <p class="appoint"><input readonly="readonly" placeholder="合约编号" value="" v-model="appointcode"></p>
...
<script>
  ...
  export default{
    ...
    attached(){
      ...
      if(this.CREDITTYPE == 3){
        this.lendname = '融资买入';
        this.action = '15012';
        this.viewname = 'view15012';
      }
      if(this.CREDITTYPE == 1){
        this.lendname = '担保品买入';
        this.action = '15010';
        this.viewname = 'view15010';
      }
      if(this.CREDITTYPE == 5){
        this.lendname = '立即买入';
        this.action = '15014';
        this.viewname = 'view15014';
        this.getAlsoCouponsFunc('checkData');
      }
      this.orderData = {};
      this.appointData = [];
    ...
    methods: {
      ...
      sendOut(){
        ...
        var sConfirm = `<p>股东账号:${oSendData.wtaccount}</p><p>证券代码:${oSendData.stockcode}</p><p>证券名称:${this.name}</p>${sPrice}<p>委托数量:${oSendData.volume}</p>${sTs}<p>确认买入该股票?</p>`;
//        $.toast("请求中！！！");
        $.confirm(
...
//          $.alert('You clicked Ok button');
      ...
      getCustomers(){
        ...
        $.showPreloader();
        service_deal_rzrq.require428(oSendData)
          .then((data)=>{
            $.hidePreloader();
            console.log(data);
            if(!data){
...
//            data && data.fiveData && that.$set('fiveData', data.fiveData);
            ...
            if(that.CREDITTYPE == 5 && that.TYPELISTINDEX != 0) return;
            if(data.FUNDTITLE){
...
//            console.log(data);
      ...
      getAlsoCouponsFunc(pdFlag){
        var that      = this;
        if(that.TYPELISTINDEX == 0 && that.orderData.length>0){
          if(that.isDisabled){
            that.isOrderList = false;
          }else {
            that.isOrderList = true;
          }
          that.isAppointList = false;
          return;
        }else if(that.TYPELISTINDEX == 1 && that.appointData.length>0){
          if(that.isAppointList){
            that.isAppointList = false;
          }else {
        ...
        $.showPreloader();
        service_deal_rzrq.require418(oSendData).then((data)=>{
            $.hidePreloader();
            ...
            data.GRID0.forEach((items,index)=>{
              ...
              var objData = {};
              if(!!listData[data['STOCKNUMINDEX']]&&listData[data['STOCKNUMINDEX']]<=0) return false;
...
//            console.log(orderData);
      ...
      orderFunc(aIndex){
        ...
        if(this.TYPELISTINDEX == 0){
          ...
          listData = this.appointData[aIndex];
          this.appointcode = listData['CONTACTINDEX'];
          this.FUNDTITLE = '合约应还数量:';
