    ...
    <div class="cont-wrap">
      <div class="cont">
        <div class="cont_left list-block ">
          ...
          <!--<div class="t1" v-on:input="clickInput"  is-stock-input="true" placeholder="请输入股票代码"  type="text"  maxlength="6"  max="6" onkeyup="this.value=this.value.replace(/\D/g,'')" value="" @click.stop="keyPanel"><em>请输入股票代码</em></div>-->
          ...
          <p class="list-price"><span class="minus" v-on:click="quantities(-1)">{{sMoney}}</span><span class="input"><span class="input-border"><span class="input-border-cont"><input type="tel" placeholder="{{volumes | getVolumes}}" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" v-model="volume"></span></span></span><span class="add" v-on:click="quantities(1)">{{sMoney}}</span></p>
...
//          this.clearData();
      ...
      focus(){
        if(this.stockcodes.length>6 && this.stockcode.length == 6){
          ...
          this.stockcodes = stock.replace(/\(.*\)/g,'').replace(/ /g,'');
        }
      },
      clickInput(data){
