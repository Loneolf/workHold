      ...
      <div class="cont_list" v-for='item in grid0' @click="getInfo($index)">
        ...
        <div class="list_con">
          <div class="w60 position">
            <p class="w50"><em class="con_title center {{item.buyDirectionColor}}">{{item.buyDirection}}</em><em class="cont_time center">{{item.ordeTime | dateForm INDEXKEY.ORDERDATEINDEX}}</em></p><p class="w50"><em class="con_wtname pgray">委托交易</em><em class="sjcj pgray">实际成交</em></p>
  ...
  export default{
    data(){
      return {
        ...
        A_ARR:[],
        S_FIELD:'',
        INDEX:['STOCKCODEINDEX','STOCKNAMEINDEX','BUYDIRECTIONINDEX','ORDERTIMEINDEX','ORDERPRICEINDEX','ORDERQTYINDEX','MATCHPRICEINDEX','MATCHQTYINDEX','DATEFORMINDEX','ORDERDATEINDEX'],
        loading:false,
        listNumber:0,
        BeginDate:'',//获取时间？
        EndDate: '',
          ...
          .then((data)=>{
            ...
            if(oData.GRID0 && oData.GRID0.length>0){
              ...
              oData.GRID0.forEach((item)=>{
                ...
                listObj.ordeTime = utils.dateForm(listObj.ORDERDATEINDEX,oData.ORDERDATEINDEX,oData.DATEFORMINDEX);//委托时间
                //$.alert(listObj.ORDERDATEINDEX)
