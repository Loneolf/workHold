<template>
  <div class="explain content native-scroll" @click="modalstock">
    <div class="cont-wrap">
      <div class="cont">
        <div class="cont_left list-block ">
          ...
          --><!--<p class="abroad-cont"><input v-on:blur="blur()"  v-on:focus="focus()" type="text" v-model="stockcodes" @input="change($event)" placeholder="请输入证券代码" maxlength="6" max="6" onkeyup="this.value=this.value.replace(/\D/g,'')"></p>-->
          ...
          <div class="list-abroad margin-bottom select_box2">
            ...
            <div v-show="isWTTYPE">
              <ul>
                <li v-for="item in WTTYPE" @click.stop="wtTypeFunc($index)" :class="{active:WTTYPEINDEX==$index}"><a href="javascript:void(0);">{{item.name}}</a></li>
              </ul>
            </div>
          </div>
          ...
          <p class="list-price"><span class="minus" v-on:click="quantities(-1)">{{sMoney}}</span><div class="t2" v-on:input="numInput"  placeholder="{{volumes | getVolumes}}"  type="number"   value="" @click.stop="keyPanel2" ><em>委托数量</em></div><span class="add" v-on:click="quantities(1)">{{sMoney}}</span></p>
          <p class="list-text">{{FUNDTITLE}}{{USABLE}}</p>
          <div class="btn" v-on:click="lendOut">立即买入</div>
    ...
    <swipe class="my-swipe" :speed="500" :auto="0" :show-indicators="true" :no-drag="true" v-ref:swipe>
      <swipe-item class="slide1"><templates url="ptjy/chicang" action="12310" viewname="view_buy"></templates></swipe-item>
...
<script>
  ...
  export default{
    ...
    attached(){
      ...
      this.clearData();
      var main =  this.main;
//      $($(this.$el).find(".t1 em")).text('请输入证券代码');
...
//        this.main.inputvalue($(this.$el).find(".t1")[0],'');
        this.main.close();
      },
      modalstock(){
        if(!!this.main){
    ...
    filters: {
      ...
      vomUnit(val){
        return utils.fmtunit(val,this.priceunit,'',true);
      },
      getVolumes(val){
        var price = val*1;
        if(typeof price == 'number' && val != ''){
          $(".t2").html("<em style='height:32px;line-height:32px;display:inline-block;border:none;font-size:14px;position:relative;'>可买:"+utils.fmtunit(val,2)+"</em>")
      ...
      filterPrice:{
        read(val){ // model -> view 在更新 `<input>` 元素之前格式化值
          ...
          if(ln>0){
            ...
            var e = v.substr(ln+1);
            var r = e.replace(/\./g,'');
            return s+''+r.substr(0,this.priceunit);
          }else if(ln == 0){
            return '0.';
          }else {
            return val;
        ...
        write(val,old){ // view -> model  在写回数据之前格式化值
          console.log(old);
          var v = ''+val;
          var ln = v.indexOf('.');
          if(ln>0){
