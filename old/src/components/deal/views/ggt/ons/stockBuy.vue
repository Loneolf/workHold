<template>
  <div class="explain content native-scroll" @click="modalstock">
    <div class="cont-wrap">
      <div class="cont">
        <div class="cont_left list-block ">
          ...
          <p class="list-price"><span class="minus" v-on:click="price(VOLUME,'-1')">{{VOLUME}}</span><span class="input"><span class="input-border"><span class="input-border-cont"><input type="number" placeholder="HK$委托价格" v-model="priceData" @blur.stop="pricebl" ></span></span></span><span class="add" v-on:click="price(VOLUME)">{{VOLUME}}</span></p>
          <p class="shouyilv">汇率：{{BUYRATE}}<!-- 添加 不含手续费提示语 -->
    ...
    <swipe class="my-swipe" :speed="500" :auto="0" :show-indicators="true" :no-drag="true" v-ref:swipe>
      <!--<swipe-item class="slide1"><templates v-if="modalShow" url="ggt/chicang" v-bind:action="16010" v-bind:viewname="view16010"></templates></swipe-item>-->
      <swipe-item class="slide1"><templates url="ggt/chicang" action="16010" viewname="view16010"></templates></swipe-item>
...
//        console.log(oSendData);
        var sConfirm = `<p>股东账号:${oSendData.wtaccount}</p><p>委托类型:${this.WTTYPENAME}</p><p>证券代码:${oSendData.stockcode}</p><p>证券名称:${this.name}</p><p>委托价格:${oSendData.price}港元</p><p>委托数量:${oSendData.volume}</p><p>确认买入该股票?</p>`;
//        $.toast("请求中！！！");
        $.confirm(
...
//          $.alert('You clicked Ok button');
      ...
      getCustomers(){
        ...
        $.showPreloader();
        service_deal_ggt.require5100(oSendData)
          .then((data)=>{
            $.hidePreloader();
            if(data.ERRORNO<0) $.alert(data.ERRORMESSAGE);
            if(data.LEVELSDATA){
              that.LEVELSDATA = data.LEVELSDATA;
              that.getVOLUMES();
            }
            ...
            that.BUYRATE = data.BUYRATE ? data.BUYRATE : '';
            that.sMoney = data.BUYUNIT ? data.BUYUNIT : '100';
            if(/*typeof data.PRICELEVELS != 'undefined'*/false){
              that.WTTYPEINDEX = data.PRICELEVELS;
            }else {
              that.WTTYPEINDEX = 0; //默认值，增强限价
            }
      ...
      getfiveData(){
        ...
        service_hq_often.require43(oSendData).then((data)=>{
            ...
            var newprice = data.WTHead.StockData.p4; //取最新价
      ...
      getVOLUMES(){
        ...
        var that = this;
        if(LEVELSDATA){
          for(var n in LEVELSDATA){
            if(parseFloat(price)>=parseFloat(LEVELSDATA[n]['DOWNPRICEINDEX']) && parseFloat(price)<parseFloat(LEVELSDATA[n]['UPPRICEINDEX'])){
              that.VOLUME = LEVELSDATA[n]['PRICESTEPINDEX'];
              return;
            }
          }
        }
      ...
      stockSxfyInfo(){
        ...
        if(!this.stockcode || !this.WTACCOUNTTYPEINDEX || !this.BUYRATE || !Number(this.BUYRATE) || Number(this.BUYRATE)<=0) return;
        //解析priceData有几位小数
        ...
        try{
          ...
          if(this.stockcode.length==5){ //港股5位
            nsyfy = Number(Number(nsyfy*Number(this.BUYRATE)).toFixed(sDh));
    ...
    watch: {
      ...
      priceData(val,oldval){
        ...
        this.getVOLUMES();
        // this.getCustomers();
        // this.getCustomers(); //可买卖数量不时时根据价格变动取
      },
      name(val){
        if(val && val != ''){
          if(this.stockcode == ''){
...
//        }
    ...
    events:{
      ...
      updateStock(code,wtTypes){
        ...
        // this.main.inputvalue($(this.$el).find(".stockcode")[0],this.stockcode);
        this.change();
      }
    },
    components:{
