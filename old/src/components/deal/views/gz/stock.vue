...
<script>
  ...
  import stockMore from './ons/stockMore.vue'
  export default{
    data(){
      return {
        currentView:'',
    ...
    methods: {
      ...
      choose(index,id,ln) {
        ...
        localService.saveMapMesg({gzstock:index});
      }
    },
    route:{
      activate ({to, from,next}) {
        var that = this;
        if(from.path){
          localService.removemapMesg(['gzstock']);
          that.inits();
          next();
        }else {
          this.selected = 3;
          this.currentView = 'view_holding';
          localService.readMapMesg(['gzstock']).then((data)=>{
            var index = data.gzstock || 0;
          that.selected = index;
