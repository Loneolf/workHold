<template>
  <div class="explain content native-scroll" @click="modalstock">
    <div class="cont-wrap">
      <div class="cont">
        <div class="cont_left list-block ">
          ...
          --><p class="abroad-cont"><input v-on:blur="blur()" v-on:focus="focus()" type="text" v-model="stockcodes" @input="change($event)" placeholder="请输入股票代码" v-bind:disabled="isDisabled" maxlength="6" onkeyup="this.value=this.value.replace(/\D/g,'')" readonly="true"></p>
          ...
          <div class="list-abroad margin-bottom"><p class="abroad-cont"><input v-on:blur="blur(1)" v-on:focus="focus(1)" type="tel" v-model="confernos" placeholder="请输入约定序号" readonly="true"></p>
          </div>
          <div class="list-abroad margin-bottom"><p class="abroad-cont"><input v-on:blur="blur(2)" v-on:focus="focus(2)" maxlength="6" max="6" type="tel" v-model="opposeatnos" placeholder="请输入对方席位号" readonly="true"></p>
          </div>
          <p class="list-price"><span class="minus grey" v-on:click="price(VOLUME*-1)">{{VOLUME}}</span><span class="input"><span class="input-border"><span class="zhezhao" v-show="isSJWT">市价委托</span><span class="input-border-cont"><input type="number" placeholder="委托价格" v-model="priceData" readonly="true"/></span></span></span><span class="add grey" v-on:click="price(VOLUME)">{{VOLUME}}</span></p>
          <p class="shouyilv">
            <span class="left">涨停：<span class="rise">{{upPrice}}</span></span><span class="right">跌停：<span class="fill">{{downPrice}}</span></span>
            <!-- 添加 不含手续费提示语 -->
    ...
    <swipe class="my-swipe" :speed="500" :auto="0" :show-indicators="true" :no-drag="true" v-ref:swipe>
      <swipe-item class="slide1"><templatesblock url="gz/yxdlist" v-bind:action="action" v-bind:viewname="viewname" v-bind:stockcode="stockcode"></templatesblock></swipe-item>
...
<script>
  ...
  import templatesblock from '../../../../common/templates-block.vue'
  ...
  export default{
    ...
    attached(){
      ...
      if(this.$route.query.stockcode){
        ...
        this.volume = (!!Number(this.$route.query.amount)?Number(this.$route.query.amount):''); //数量
        this.opposeatnos = '对方席位号:'+this.$route.query.oppseatno;;//席位编号
        this.opposeatno = this.$route.query.oppseatno;//席位编号
        this.confernos = '约定序号:'+this.$route.query.conferno; //约定号
        this.conferno = this.$route.query.conferno;//约定号
      }
...
//          CREDITTYPE:this.CREDITTYPE,
        ...
        var sConfirm = `<p>股东账号:${oSendData.wtaccount}</p><p>证券代码:${oSendData.stockcode}</p><p>证券名称:${this.name}</p><p>委托价格:${oSendData.price}</p><p>委托数量:${oSendData.volume}</p><p>约定序号:${oSendData.conferno}</p><p>对方席位号:${oSendData.opposeatno}</p>${sTs}<p>确认买入该股票?</p>`;
//        $.toast("请求中！！！");
        $.confirm(
...
//          $.alert('You clicked Ok button');
            ...
            service_gz.require178(oSendData)
              .then((data)=>{
                $.hidePreloader();
                $.alert(data.ERRORMESSAGE,function(){
                    that.$TZT.action10002(); //返回
                });
                if(data.ERRORNO>=0){
                  that.stockcodes = '';
        ...
        service_deal_often.require150(oSendData)
          .then((data)=>{
            ...
            if(data && data.wtaccountlist){
              ...
              that.$set('WTACCOUNTNAME',data.wtaccountlist[that['WTACCOUNTINDEX']]['wtaccount']+'('+this.wtaccountlist[that['WTACCOUNTINDEX']]['name']+')');
            }
...
//            data && data.fiveData && that.$set('fiveData', data.fiveData);
    ...
    events:{
      ...
      updateStock(obj){
        this.stockcodes = obj.stockcode;//股票代码
        this.change();
        this.priceData = obj.price;//价格
        this.newPrice = obj.price;//价格
        this.volume = (!!Number(obj.amount)?Number(obj.amount):''); //数量
        this.opposeatnos = '对方席位号:'+obj.oppseatno;//席位编号
        this.opposeatno = obj.oppseatno;//席位编号
        this.confernos = '约定序号:'+obj.conferno; //约定号
        this.conferno = obj.conferno;//约定号
      }
    },
    components:{
      Swipe,
