    ...
    <div class="cont-wrap">
      <div class="cont">
        <div class="cont_left list-block ">
          ...
          --><!--<p class="abroad-cont"><input v-on:blur="blur()" v-on:focus="focus()" type="text" v-model="stockcodes" @input="change($event)" placeholder="请输入股票代码" maxlength="6" onkeyup="this.value=this.value.replace(/\D/g,'')"></p>-->
  ...
  export default{
    ...
    attached(){
      ...
      if(type == 4 || type == 3){
        ...
        this.lendname = '定价卖出';
        this.action = '13015';
        this.viewname = 'view13015';
      }
      if(type == 2 || type == 1){
        this.WTPriceType = 'a';
        this.lendname = '限价卖出';
        this.action = '13011';
        this.viewname = 'view13011';
      }
      this.stockcode = '';
      this.stockcodes = '';
    ...
    methods: {
      ...
      getWTTYPE(){
        ...
        service_deal_often.require5124(obj)
          .then((data)=>{
      ...
      sendOut(){
        ...
        var sConfirm = `<p>股东账号:${oSendData.wtaccount}</p><p>证券代码:${oSendData.stockcode}</p><p>证券名称:${this.name}</p>${sPrice}<p>委托数量:${oSendData.volume}</p>${sTs}<p>确认卖出该证券?</p>`;
//        $.toast("请求中！！！");
        $.confirm(
...
//        this.WTPriceType = 'a';
    ...
    watch: {
      ...
      priceData(val,old){
        ...
        var newdata = v;
        if(ln>0){
          var s = v.substr(0,ln+1), //取小数点前面包含小数点
              r = v.substr(ln+1).replace(/\./g,''); //取小数点后面数据
          newdata = s+''+r.substr(0,this.priceunit);
        }else if(ln == 0){ //这个是错误的，会破坏数据字符值，不知道为什么会这样
          newdata = '0.';
        }else {
          if(newdata.length>1){
