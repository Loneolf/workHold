<template>
  <div class="cont_list" v-for="items in gridData" @click="listFunc($index)">
    <p class="list_title"><span class="w50"><em class="name">{{items.STOCKNAMEINDEX}}</em><em class="code">{{items.STOCKCODEINDEX}}</em></span><span class="w50 category"><em class="chedan " @click.stop.prevent="cancelFunc($index)">点击撤单</em></span></p>
    <div class="list_con"><div class="w50 position"><p class="w50 pt15 center"><em class="con_title {{items.BUYDIRECTIONINDEX.indexOf('买')>-1?'red':'blue'}}">{{items.BUYDIRECTIONINDEX}}</em><em class="cont_time">{{items.ORDERTIMEINDEX | dateForm INDEXKEY.ORDERTIMEINDEX}}</em></p><p class="w50"><em class="con_wtname pgray">委托状态</em><em class="con_wtname pgray">委托交易</em><em class="sjcj pgray">实际成交</em></p><span class="vertical h60"></span></div><p class="w50"><span class="accountright wtamount">{{items.ENTRUSTTYPENAMEINDEX}}</span><span class="accountright wtamount">{{items.ORDERPRICEINDEX}}/{{items.ORDERQTYINDEX}}股</span><span class="accountright con_sjamount">{{items.MATCHPRICEINDEX}}/{{items.MATCHQTYINDEX}}股</span></p></div>
...
<script>
  ...
  import util from '../../../../util/util'
	export default{
        ...
        gridData:[],
        loading:false,
        INDEX: ['STOCKCODEINDEX','STOCKNAMEINDEX','ENTRUSTTYPENAMEINDEX','ORDERPRICEINDEX','ORDERQTYINDEX','ORDERTIMEINDEX','MATCHPRICEINDEX','MATCHQTYINDEX','DRAWINDEX','BUYDIRECTIONINDEX','CONTACTINDEX'],
        INDEXKEY:{},
        listNumber:0,
        S_FIELD:'',
        A_ARR:[],
    ...
    methods: {
      ...
      loadMore () {
        ...
        service_gz.require179(oSendData)
          .then((data)=>{
            console.log(data);
            $.hidePreloader();
            var ArrayData = [];
            ...
            data.GRID0.forEach((item,index) => {
              ...
              that.INDEX.forEach((value) => {
                if (data[value]) {
                  if(value == 'BUYDIRECTIONINDEX'){
                    if(listItem[11] == '互报成交确认委托'){
                      listobj[value] = '互报成交确认'+listItem[data[value]];
                      listItem[data[value]] = listobj[value];
                    }else if(listItem[11] == '确认委托'){
                      listobj[value] = '成交确认'+listItem[data[value]];
                      listItem[data[value]] = listobj[value];
                    }else if(listItem[11] == '定价委托'){
                      listobj[value] = '定价'+listItem[data[value]];
                      listItem[data[value]] = listobj[value];
                    }else if(listItem[11] == '限价委托'){
                      listobj[value] = '限价'+listItem[data[value]];
                      listItem[data[value]] = listobj[value];
                    }
                  }else{
              ...
              data.GRID0[index] = listItem.join('|');
