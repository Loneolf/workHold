<template>
  <div class="cont_list" v-for="item in grid0" @click="listFunc($index)">
    <p class="list_title">
      ...
      <span class="w50 category"><em class={{item.draw}} @click.stop.prevent="doChedan($index)">点击撤单</em></span>
    </p>
    <div class="list_con">
      <div class="w60 position">
        <p class="w50 pt15"><em class="cont_time center">{{item.orderTime}}</em></p>
        <p class="w50"><em class="con_wtname pgray">委托状态</em><em class="con_wtname pgray">委托数量</em><em class="con_wtname pgray">委托价格</em></p>
