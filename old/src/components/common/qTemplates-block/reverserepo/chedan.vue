<template>
  <div class="cont_list" v-for="item in grid0" @click="listFunc($index)">
    <p class="list_title">
      <span class="w50"><em class="name">{{item.name}}-{{item.stockName}}&nbsp;{{item.stockCode}}</em></span><!-- <span class="w50 category"><em class={{item.draw}} @click.stop.prevent="doChedan($index)">撤单</em></span> -->
    </p>
    <div class="list_con">
      <div class="w60 position">
       <!--  <p class="w50 pt15"><em class="cont_time center">{{item.orderTime}}</em></p> -->
       <p class="w50"><em class="con_wtname pgray pgrays">委托金额：{{item.orderQty}}</em><em class="con_wtname pgray pgrays">委托利率：{{item.orderPrice}}</em></p>
        ...
        <p class="w50"><em class="con_wtname pgray">委托时间：{{item.orderTime}}</em></p>
        <span class="vertical h60"></span>
        <p class="cd"><!-- <span class="done">已成</span> --><span class={{item.draw}} @click.stop.prevent="doChedan($index)">{{item.cancelState}}</span></p>
      </div>
  ...
  export default{
    data(){
      return {
        grid0:[],
        A_ARR:[],
        INDEX:['STOCKCODEINDEX','STOCKNAMEINDEX','ENTRUSTTYPENAMEINDEX','ORDERPRICEINDEX','ORDERQTYINDEX','ORDERTIMEINDEX','DRAWINDEX','DATEFORMINDEX','CONTACTINDEX','ENTRUSTSTATUSINDEX'],
        loading:false,
        S_FIELD:'',
        ACTIONS:{},
        INDEXKEY:{},
...
//              console.log(listObj);
                ...
                listObj.orderQty = listObj.ORDERQTYINDEX*100;
                listObj.orderTime = util.dateForm(listObj.ORDERTIMEINDEX,oData.ORDERTIMEINDEX,oData.DATEFORMINDEX);
                listObj.wtzt = listObj.ENTRUSTSTATUSINDEX;
               // listObj.cdbz = listObj.DRAWINDEX;
                console.log(listObj.wtzt);
                if(listObj.wtzt==6){
                   listObj.cancelState = "已撤";
                   listObj.draw = 'gray';
                }else if(listObj.wtzt==8){
                  listObj.cancelState = "已成";
                   listObj.draw = 'gray';
                }else{
                  listObj.cancelState = "撤单";
                    listObj.draw = 'chedan';
                };
      ...
      doChedan(index){
        ...
        if(arrData.DRAWINDEX=='0'){
            alert('该笔委托不可撤单!');
             return;
        }
        var name = repoconfig.repoData[arrData.stockCode]+'天期';
        let oSendData = {
          ContactID: arrData.CONTACTINDEX
        };
        var aWord = [];
        aWord.push('<p>代码:' + arrData.stockCode +'</p>');
        aWord.push('<p>名称:' + name  + '</p>');
        aWord.push('<p>价格:' + arrData.orderPrice + '</p>');
        aWord.push('<p>金额:' + arrData.orderQty +  '</p>');
      ...
      listFunc(index){
       let oSave = {
        ...
        localService.saveMapMesg({C_DETAIL: JSON.stringify(oSave)})
         // .then((data)=>{
          //if(data.ERRORNO>=0){
            this.$TZT.action10061({url:'/deal/details?num='+index});
