<template>
  <div class="table">
    <div v-for="item in gridData" track-by="$index" @click="listFunc($index)" class="table-list {{item | isBuy YKINDEX}}">
      <p v-for="trItem in dateArray"><span v-for="dataList in trItem" track-by="$index" class="{{$index | textAgline trItem.length | setFont dataList item}}">{{dataList | valueKey item}}<em class="ggtIcon {{dataList | wtTypeKey item}}"></em></span></p>
    </div>
  </div>
</template>
...
//            that.$set('gridData',data.GRID0);
      ...
      listFunc(index){
        ...
        if(this.action){
          this.$TZT.tradeaction({action:this.action,stockcode:stockcode,wtaccounttype:wtaccounttype},this);
        }
      }
    },
    filters: {
