<template>
  <div>
    <div class="h5_difference" style="width: 100%;height: 150px;padding: 0;" id="component">
    ...
    <div class="h5_content">
      <p><span class="cont-left"><span class="select" v-on:click="selectFunc">{{selectName}}<span class="dot-bottom"></span></span><!--
        --><span class="title" v-show="VOLTitle"><span class="line1">MA5:{{Line1}}</span><span class="line2">MA10:{{Line2}}</span><span class="line3">MA20:{{Line3}}</span></span>
        <span class="title" v-show="MACDTitle"><span class="line1">MACD(12,26,9):{{Line1}}</span><span class="line2">DIF:{{Line3}}</span><span class="line3">DEA:{{Line2}}</span></span>
        <span class="title" v-show="KDJTitle">KDJ(9,3,3)<span class="line1">K:{{Line1}}</span><span class="line2">D:{{Line2}}</span><span class="line3">J:{{Line3}}</span></span>
        <span class="title" v-show="RSITitle">RSI(6,12,24)<span class="line1">RSI6:{{Line1}}</span><span class="line2">RSI12:{{Line2}}</span><span class="line3">RSI24:{{Line3}}</span></span>
        <span class="title" v-show="WRTitle"><span class="line1">WR(10):{{Line1}}</span></span>
        <span class="title" v-show="BOLLTitle">BOLL(20,2)<span class="line1">MID:{{Line1}}</span><span class="line2">UP:{{Line2}}</span><span class="line3">LOW:{{Line3}}</span></span>
        <span class="title" v-show="CCITitle">CCI(14)<span class="line1">:{{Line1}}</span></span>
        <span class="title" v-show="ROCTitle">ROC(12)<span class="line1">ROC:{{Line1}}</span><span class="line2">MAROC:{{Line2}}</span></span>
        <span class="title" v-show="ASITitle">ASI<span class="line1">:{{Line1}}</span></span>
        <span class="title" v-show="WVADTitle">WVAD(6)<span class="line1">:{{Line1}}</span></span>
        <span class="title" v-show="BIASTitle">BIAS(6,12,24)<span class="line1">6:{{Line1}}</span><span class="line2">12:{{Line2}}</span><span class="line3">24:{{Line3}}</span></span>
        <span class="title" v-show="EMVTitle">EMV(14,9)<span class="line1">EMV:{{Line1}}</span><span class="line2">EMVA:{{Line2}}</span></span>
        <span class="title" v-show="DMITitle">DMI(7)<span class="line1">DI1:{{Line1}}</span><span class="line2">DI2:{{Line2}}</span><span class="line3">ADX:{{Line3}}</span><span class="line4">ADXR:{{Line4}}</span></span>
        <span class="title" v-show="DMATitle">DMA(10,50)<span class="line1">DMA:{{Line1}}</span><span class="line2">AMA:{{Line2}}</span></span>
        <span class="title" v-show="TRIXTitle">TRIX(12,20)<span class="line1">TRIX:{{Line1}}</span><span class="line2">TRMA:{{Line2}}</span></span>
        <span class="title" v-show="BRARTitle">BRAR(26,0)<span class="line1">BR:{{Line2}}</span><span class="line2">AR:{{Line1}}</span></span>
        <span class="title" v-show="VRTitle">VR(24)<span class="line1">:{{Line1}}</span></span>
        <span class="title" v-show="OBVTitle">OBV<span class="line1">:{{Line1}}</span></span></span><!--
        <span class="fuquan">前复权</span>-->
      </p>
      <p class="select-vule" v-show="is_select"><span v-for="v in cfg1.stockIndex" v-on:click="vuleFunc($index)">{{v}}</span></p>
    </div>
...
//    date_canvas();
        ...
        cfg.component_index.unbind().on('click',function () {
          var index = cfg.stockIndex.indexOf(cfg.stockVal);
          if(index>=0 && index<cfg.stockIndex.length-1){
            cfg.stockVal = cfg.stockIndex[index+1];
            that.stockIndexList(cfg.stockIndex[index+1]);
          }else {
            cfg.stockVal = '成交量';
            that.stockIndexList('成交量');
          }
        });
        if (browser.isMobile()) {
          var endTime = 0;
      ...
      stockIndexList(index) {
        ...
        switch (index) {
          case '成交量':// Volume
            ...
            GiscHqTechCanvas.chengjiaol_canvas(ctx_index,this.cfg1.data.MAXCOUNT,this.cfg1.data.TkLineData,o_w,addCount,up_color,low_color,color_line,h_index,total_maxs,this.listVol,this.cfg1.data.CONTACTID);
            //成交量均线
