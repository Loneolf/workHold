<template>
  <div class="chart-wrapper" style="height: 100%;">
    ...
    <div class="h5_date" id="component_date">
      <div style="position: relative; font-size: 0px">
        <canvas style="z-index: 1;" id="cns_date"></canvas>
      </div>
    </div>
  </div>
</template>
<script>
  ...
  import GiscHqTechCanvas from '../../scripts/GiscHqTechCanvasNew';
  export default {
    ready(){
    ...
    methods:{
      H5Difference(cfg) {
        ...
        var h_date      = cfg.height_date;
        var b           = 0;//留20像素写时间位置
        var color_b     = '#F5F6F9';
        var color_line  = '#fff';
        var c_average   = '#6D7A85';
        var up_color    = '#E83333';
        var low_color   = '#129E6B';
        ...
        var average_c_a = ['#E26C1B','#9C50B3','#4C78C7','#529C4F'];
        var k_val       = 9;
...
//  console.log(maxVal,min,maxRose,minRose);
        ...
        function kline_canvas() {
          ...
          klineData.forEach((item,index)=>{
            ...
            // }
            // ctx_index.font = "20px Helvetica";
            // ctx_index.fillStyle = c_average;
            // ctx_index.textAlign = "left";
            // if(index == 0){
            //   ctx_index.fillText(util.formatTimeLable(item.Time,that.StockMinIndex),12,h-3);
            // }
            // if(index == Math.floor((parseFloat(that.listVol))/2)){
            //   ctx_index.fillText(util.formatTimeLable(item.Time,that.StockMinIndex),x_b-40,h-3);
            // }
            // if(index == that.listVol-1){
            //   ctx_index.textAlign = "right";
            //   ctx_index.fillText(util.formatTimeLable(item.Time,that.StockMinIndex),x_b,h-3);
            // }
            date_canvas(index, item, x_b, h_date);
//        chengjiaol_canvas(item,x_b,o_w,color);
        ...
        function text_canvas() {
          ...
          ctx.fillText(h_p,10,20);
          ctx.fillText(l_p,10,kLine_h-5);
        }

        function date_canvas(index, item, x_b, h) {
          //日期
          ctx_date.font = "20px Helvetica";
          ctx_date.fillStyle = c_average;
          ctx_date.textAlign = "left";
          if(index == 0){
            ctx_date.fillText(util.formatTimeLable(item.Time,that.StockMinIndex).replace(/\//g, '-'),10,h-3);
          }
          if(index == Math.floor((parseFloat(that.listVol))/2)){
            ctx_date.fillText(util.formatTimeLable(item.Time,that.StockMinIndex).replace(/\//g, '-'),x_b-40,h-3);
          }
          if(index == that.listVol-1){
            ctx_date.textAlign = "right";
            ctx_date.fillText(util.formatTimeLable(item.Time,that.StockMinIndex).replace(/\//g, '-'),x_b,h-3);
          }
        }
        ...
        //新建画布上一层
        var cns_date = cfg.cns_date;
        var ctx_date = cns_date.getContext('2d');
        cns_date.width = cns_date.width = w;
        cns_date.height = cns_date.height = h_date;
      ...
      stockIndexList(index) {
        ...
        switch (index) {
          case '成交量':// Volume
            ...
            GiscHqTechCanvas.DrawText(ctx_index,nMaxDelta,'','#6D7A85',h_index); //原来''表示0  因测试人员说要优化这个固直接将0 写成空
        ...
        if(index !== '成交量'){
          ...
          GiscHqTechCanvas.DrawText(ctx_index,max,min,'#777777',h_index);
          if(Line1){
    ...
    events:{
      'parent-chartKLinedata':function(chartdata){
        ...
        var Dom_date = $(this.$el).find('.h5_date');
        ...
        this.cfg1.height_date = Dom_date.height() * 2;
        ...
        this.cfg1.cns_date = $(this.$el).find('#cns_date')[0];
        this.cfg1.component_date = $(this.$el).find('#component_date');
        try{
          this.H5Difference(this.cfg1);
        }
        catch(e){
          var r = this.cfg1.cns.getContext("2d");
          r.clearRect(0, 0, this.cfg1.width, this.cfg1.height); 
          r = this.cfg1.cnst.getContext("2d");
          r.clearRect(0, 0, this.cfg1.width, this.cfg1.height);
          var r_index = this.cfg1.cns_index.getContext("2d");
          r_index.clearRect(0, 0, this.cfg1.width, this.cfg1.height_index);
          r_index = this.cfg1.cnst_index.getContext("2d");
          r_index.clearRect(0, 0, this.cfg1.width, this.cfg1.height_index);
          var r_date = this.cfg1.cns_date.getContext("2d");
          r_date.clearRect(0, 0, this.cfg1.width, this.cfg1.height_date);
        }
      }
    }
  }
...
<style lang="less" scoped>
  ...
  .h5_difference canvas ,.h5_quota canvas,.h5_date canvas{
    width: 100%;
    height: 100%;
  }
  .h5_content{
