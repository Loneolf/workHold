<template>
  <div class="fund_table" id="lzgList">
      <dl class="part" v-for="items in lzgModule" v-on:click="clickFunc($index)">
        ...
        <dd><span class="{{items.UPDOWNPINDEX | isUP}}">{{items.NEWPRICEINDEX}}</span></dd><dd><span>{{items.CHANGEPERINDEX}}</span></dd>
      </dl>
  </div>
</template>
<script>
  ...
  import tztStockType from '../../../hq/scripts/tztStockType.js';
	export default {
        ...
        NEWMARKETNOArray:'',
        lzgModule:[
          {STOCKNAMEINDEX:'--',UPDOWNPINDEX:'--',NEWPRICEINDEX:'--',STOCKCODEINDEX:'--',CHANGEPERINDEX:'--'},
          {STOCKNAMEINDEX:'--',UPDOWNPINDEX:'--',NEWPRICEINDEX:'--',STOCKCODEINDEX:'--',CHANGEPERINDEX:'--'},
          {STOCKNAMEINDEX:'--',UPDOWNPINDEX:'--',NEWPRICEINDEX:'--',STOCKCODEINDEX:'--',CHANGEPERINDEX:'--'},
          {STOCKNAMEINDEX:'--',UPDOWNPINDEX:'--',NEWPRICEINDEX:'--',STOCKCODEINDEX:'--',CHANGEPERINDEX:'--'},
          {STOCKNAMEINDEX:'--',UPDOWNPINDEX:'--',NEWPRICEINDEX:'--',STOCKCODEINDEX:'--',CHANGEPERINDEX:'--'},
    ...
    methods: {
      getlzgData(){
        ...
        service_hq_often[action](oSendData)
          .then((data)=> {
            ...
            that.NEWMARKETNOArray = data.NEWMARKETNO;
            let indexData = [],
                STOCKPROP = (data.STOCKPROP?data.STOCKPROP.split('|'):[]);
            if(action == 'require20199'){
              that.isBK = true;
              data.GRID0.shift();
              STOCKPROP.shift();
            }
            data.GRID0.forEach((items,index)=>{
              if(index > 0){
                ...
                let listObj = {
                  ...
                  CHANGEPERINDEX:listData[data.CHANGEPERINDEX],
      ...
      clickFunc(index){
        var grid = this.lzgModule;
        if(index>=grid.length){
          return;
        }
        var oData = grid[index];
        var marketno = this.NEWMARKETNOArray.split('|')[index];
        if(this.isBK){
          marketno = this.NEWMARKETNOArray.split('|')[index+1];
        }
        this.$TZT.action1964(`/hq/ggStockInfo?stockcode=${oData.STOCKCODEINDEX}&marketno=${marketno}`);
      }
    },
    events:{
