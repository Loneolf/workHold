<!-- 资金流向 -->
<template>
    <div class="content" v-el:content-el>
...
export default {
    ...
    methods: {
        ...
        getCustomers() {
            ...
            that.marketno = (that.$route.query.marketno?that.$route.query.marketno:that.$route.query.stockcodetype); //兼容市场
            var obj = {
              stockcode: that.stockcode,
              NewMarketNo:that.marketno, //股票市场
              StartPos:999999
            };
            service_hq_often.require20131(obj)
                .then((oData) => {
                    ...
                    this.$nextTick(function(){
                        var obj={};
                            obj.contentheight=this.$els.contentEl.offsetHeight;
                        //避免重复调用客户端的设置高度
                        if(browser.browser.versions.webview && obj.contentheight>that.pageHeight){
                            that.pageHeight = obj.contentheight;
                            localService.readLocalMesg(["tztwkwebview"])
                               .then((data)=>{
        ...
        notifyRing(GRID0) {
            this.$broadcast('parent-ring', GRID0)
        }
    },
    components:{
        componentRing,
