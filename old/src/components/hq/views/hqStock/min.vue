<template>
  <div class="swipes" style="width: 100%;height: 265px;">
    <p class="time-title" v-show="isshow"><span>{{times}}</span><span>价:<em class="{{Last_p | colorDivide Close_p}}">{{Last_p}}</em></span><span>涨跌:<em class="{{m_lUpPrice | isUP}}">{{m_lUpPrice}}</em></span><span>涨幅:<em class="{{m_lUpPrice | isUP}}">{{m_lUpIndex}}</em></span><span>量:{{total_h}}</span></p>
    <div v-show="bIndex">
      <p class="time-text" v-if="isZS">{{zsname}}<span class="lx">{{m_lx}}</span></p>
      <p class="time-text" v-else>均价<span>{{m_lAvgPrice}}</span> 现手<span>{{Last_h}}</span></p>
    </div>
    <time></time>
  </div>
</template>
