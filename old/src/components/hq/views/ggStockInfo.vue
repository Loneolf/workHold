<template>
  <div>
    ...
    <div class="content" @click="modalstock">
      <div class="content">
        <!--v-pull-to-refresh="loadMore"-->
        <component :is="currenthead" keep-alive></component>
        <div v-if="isZqShow" class="zq_box">
          <p>回购期限：{{ RepurDay }}</p>
          <p>
            实际占款天数：<span class="red">{{ possesDay }}</span>
          </p>
        </div>
        <div v-if="isGzShow" class="zq_box">
          <p>
            转让状态：<span class="red">{{ gzZrStateName }}</span>
          </p>
          <p>
            转让类型：<span class="red">{{ gzZrTypeName }}</span>
          </p>
        </div>
        <div v-if="isGzYxgShow" class="zq_box">
          <p
            ...
            "
          >
            起息日：<span class="red">{{ yxgData.valueDate }}</span>
          </p>
          <p
            style="
              font-size: 12px;
          ...
          <p
            ...
            "
          >
            {{ yxgData.isKezhuangg == "N" ? "不可转股" : "可转股" }}
          </p>
          <p
            style="
              font-size: 12px;
            ...
            "
          >
            票面股息率：<span class="red">{{ yxgData.dividendYield }}</span>
          </p>
        </div>
        <div class="AH" v-show="isAH" @click="AHclick">
          <span class="left"
            ><span class="AHname">{{ AHData.name }}</span
            ><span class="AHprice {{AHData.m_AHRatio | isUP}}">{{
              AHData.m_AHPrice
            }}</span
            ><span class="AHfudu {{AHData.m_AHRatio | isUP}}">{{
              AHData.m_AHRatio
            }}</span></span
          ><span class="right">溢价(A/H):{{ AHData.m_AHPremiumRate }}</span>
        </div>
        <div class="stock_tab chart_tab">
          <ul>
            <li
              v-for="canvasTab in canvasTabs"
              @click.stop="choose($index, canvasTab.id)"
              :class="{ active: $index === selected }"
            >
              <a href="javascript:void(0);" id="{{canvasTab.id}}"
                >{{ canvasTab.tabName
                }}<span class="dot-bottom" v-if="$index > 4"></span
              ></a>
            </li>
          </ul>
          <p class="select-vule" v-show="isMins">
            <span
              v-for="items in minsData"
              @click.stop="getMins($index)"
              :class="{ active: $index === selectedMins }"
              >{{ items.name }}</span
            >
          </p>
        </div>
        ...
        <div class="stock_tab" v-show="isView1">
          <ul>
            <li
              v-for="canvasTab in Tabs1"
              @click="choose($index, canvasTab.id, 1)"
        ...
        <div class="stock_tab" v-show="isView2">
          <ul>
            <li
              v-for="canvasTab in Tabs2"
              @click="choose($index, canvasTab.id, 2)"
...
import HKHead from "../ons/stockInfo/HKHead.vue";
import HKZHead from "../ons/stockInfo/HKZHead.vue";
  ...
  methods: {
    dealRefreshTimer() {
      ...
      this.$refs.ref2 &&
        ...
        this.$refs.ref2.queryKLineData();
      this.$refs.ref3 &&
        this.$refs.ref3.dealRefreshTimer &&
        this.$refs.ref3.dealRefreshTimer();
      this.$refs.ref4 &&
        this.$refs.ref4.getlzgData &&
        this.$refs.ref4.getlzgData();
      this.$refs.ref4 &&
        this.$refs.ref4.getCustomers &&
        this.$refs.ref4.getCustomers();
      this.$refs.ref1 &&
        this.$refs.ref1.getCustomers &&
        this.$refs.ref1.getCustomers();
    },
    inits(marketno) {
      //      console.log(this);
    ...
    getEXtHqType() {
      ...
      if (tztStockType.MakeHKMarket(this.NEWMARKETNO)) {
        ...
        view = "QHHead";
      } else if (tztStockType.MakeWPMarket(this.NEWMARKETNO)) {
        //外盘
        view = "HKZHead";
      } else {
        if (
          tztStockType.MakeIndexMarket(this.NEWMARKETNO) ||
    ...
    queryHKMinDataTime() {
      ...
      console.log('请求前',this.NEWMARKETNO)
      service_hq_often.require20109(oSendData).then((data) => {
        ...
        console.log('进程1',this.NEWMARKETNO)
       ...
       if (isShzy || isSzzy) {
        ...
        console.log('this.NEWMARKETN2',this.NEWMARKETNO)
        if (
          tztStockType.MakeStockMarketStock(this.NEWMARKETNO) &&
          ...
          if (!data.quoteAdd) {
            ...
            console.log('显示',that.gzZrStateName,that.gzZrTypeName)
          } else {
            console.log('第二个灭幼如果')
