...
/**
 ...
 @param status 1 是否CDR股票 2 是否沪伦通CDR 
 @return true 
 */
const MakeCDRMarket = function (stockProp, status) {
  var nsCDR = "", cCDR = ""
  if (stockProp.length > 22) {
    nsCDR = stockProp.slice(22, 23);
  }
  if (nsCDR.length < 1) return false;
  cCDR = nsCDR;
  if (nsCDR.length > 0 && cCDR[0] == status) return true;
  return false;
}
...
export default {
    ...
    MakeyxgMarket:MakeyxgMarket,
    MakeCDRMarket: MakeCDRMarket
}
