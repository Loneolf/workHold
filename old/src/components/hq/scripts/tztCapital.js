...
Capital.prototype.init = function(option){
    ...
    if(this.options.$canvas.length<=0) return;
...
Capital.prototype.initcanvas = function(){
    var canvas = this.options.$canvas[0],
        ctx = canvas.getContext("2d");
    if(!this.options.csHeight||isNaN(this.options.csHeight)) return console.log('高度必须指定:options.csHeight');
    this.options.csWidth = (this.options.csWidth?this.options.csWidth:document.body.clientWidth); //兼容处理
    var x = this.options.csWidth*2,
        y = this.options.csHeight*2;
    canvas.width = this.options.csWidth*2; //设置高度
    canvas.height = this.options.csHeight*2; //设置高度
    this.options.$canvas.css({'width':this.options.csWidth+'px','height':this.options.csHeight+'px'});
