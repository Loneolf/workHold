...
function chengjiaol_canvas1(canvasID, MAXCOUNT, TkLineData, o_w, addCount, up_color, low_color, color_line, h_index, total_max, listVol, CONTACTID) {
  // 成交柱状
  var klineData = TkLineData;
  if (MAXCOUNT == klineData.length - 1 || TkLineData.length > listVol) {
    klineData = klineData.slice(addCount);
...
function DrawText(canvasID, nMaxDelta, lMinValue, nColor, h_index) {
  ...
  canvasID.fillText(nMaxDelta, 10, 20);
  canvasID.fillText(lMinValue, 10, h_index - 5);
}
