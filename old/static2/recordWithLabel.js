(function(win){
	...
	function maidianObj(){
		...
		*/
		...
		/**
		 //参数：
		 //ranPara:随即参数，格式为{baseVal:10000,randomVal:10}，对象，成员变量都是int类型
		 //action:事件埋点字符串
		 //param：事件埋点属性的对象{属性1：属性值1,属性2：属性值2,...}
		 **/
		this.reportEventRandom = function(ranPara, action, param){
			if (maidianFlg) {
				try{
					var paramCheck = ranPara instanceof Object && ranPara.baseVal!=undefined && ranPara.randomVal!=undefined && typeof ranPara.baseVal=='number' && typeof ranPara.randomVal=='number';
					if (paramCheck && parseInt(Math.random() * ranPara.baseVal)<parseInt(ranPara.randomVal)){
