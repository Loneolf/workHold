...
window.alert = function(msg){
    // 文本转换 配置列表 regexNo 需要匹配的正则 transcode 转换后的文本
    var errTranscodeList_TZT = [
    
        {
            regexNo:/无此功能\[/, // 20241011 无此功能[XXXX]，需要转换为网络异常
            transcode:'网络异常'
        },
        {
            regexNo:/系统忙，缓存空间已满/,  // 20241011 系统忙，缓存空间已满，需要转换为网络不佳，请稍后重试
            transcode:'网络不佳，请稍后重试'
        },
        {
            regexNo:/业务受理中，请稍后检查是否成功/, // 20241011 业务受理中，请稍后检查是否成功，需要转换为网络不稳定，请稍后检查是否成功
            transcode:'网络不稳定，请稍后检查是否成功'
        },
        {
            regexNo:/路由不能转发/, // 20241011 路由不能转发[转发错误],发送者信息XXXX，需要转换为网络存在异常
            transcode:'网络存在异常'
        },
    ];
    // 文本转换函数 errorInfo 需要转换的文本
    function errTranscode_TZT(errorInfo){
        var retuenText = errorInfo;//默认文本
        try {//错误文本转换
            for(var i=0;i<errTranscodeList_TZT.length;i++){//循环匹配
                if(errTranscodeList_TZT[i].regexNo.test(errorInfo)){//匹配
                    retuenText = errTranscodeList_TZT[i].transcode;//返回转换后的文本
                    break;//退出循环
                }
            }
        } catch (error) {//转换异常
        ...
        return retuenText;//返回文本
    };

    if(typeof msg === 'string'){
        msg = errTranscode_TZT(msg);
    }
    window._alert(msg);
}
...
H5TZT.readLocalMesgSync = function(sArray, fnSuccess){
    var sSendURL = "",
        oThis = this,
        isSlice=true;
    if(typeof sArray == 'string'){
    ...
    H5TZT.tztWebdataEncrypt(config,sSendURL,'H5SetDataEncryptSync');
}
...
H5TZT.saveMapMesgSync = function(obj, fnSuccess) {
    var sSendURL = "",
        oThis = this,
        isSlice=true;
    if(typeof obj == 'string'){
...
H5TZT.readMapMesgSync = function(sArray, fnSuccess) {
    var sSendURL = "",
        oThis = this;
    if(typeof sArray != 'string'){
        for (var x = 0; x < sArray.length; x++) {
...
H5TZT.saveFileMesgSync = function(saveMesg, fileName, fnSuccess) {
    var SAVEDATA = '';
    if(typeof saveMesg == 'string'){
        SAVEDATA = saveMesg;
    }else{
    ...
    H5TZT.tztWebdataEncrypt(config,encodeURI(SAVEDATA),'H5SetDataEncryptSync');
}
...
H5TZT.readFileMesgSync = function(fileName, fnSuccess) {
    var sSendURL = "/reqreadfile?filename=" + fileName;
    var config = {
        newUrl:sSendURL,
        fnSuccess:fnSuccess,
    ...
    H5TZT.tztWebdataEncrypt(config,'','H5SetDataEncryptSync');
}
...
window.H5SetDataEncryptSync = function(newData,config){
    if(typeof config != 'object' && config){
        config = H5TZT.tztwkwebviewObj[config] || {};
    }
    //发送请求
